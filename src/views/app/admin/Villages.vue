<template>
    <section id="villages" class="section-main">
        <div class="title-row flx gap-50 ai-c jc-sb">
            <div class="flx gap-16 ai-c">
                ...
                <div class="input-wrapper">
                    ...
                    <input class="form-control" type="search" data-type="icon" placeholder="Search village...">
                </div>
            </div>
            <button @click="$store.commit('openModal', 'add-village')" class="button-primary gap-8 btn-md">
        ...
        <div class="grid village-grid gap-16 grid-col-4">
            <village-list v-for="village in villages" :key="village.id" :village="village" />
        </div>
    </section>
</template>
<script>
