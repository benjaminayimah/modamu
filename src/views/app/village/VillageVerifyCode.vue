    ...
    <section class="section-main">
        <div class="title-row flx gap-50 ai-c">
            ...
            <div class="input-wrapper">
                ...
                <input v-model="search" class="form-control" type="search" data-type="icon" placeholder="Name, gender...">
            </div>
        </div>
        <div class="table" id="village_checkout">
            <div class="grid gap-8 table-body">
                ...
                <request-list-row v-else-if="displayedItems.length" v-for="attendee in sort_newest(displayedItems) " :key="attendee.id" :attendee="attendee" @check-in="checkIn" :creating="creating"/>
...
export default {
    components: { RequestListRow, PaginationControls },
    ...
    computed: {
        ...
        computedItem() {
            ...
            return this.attendees.filter(item => {
                return item.kid_name.toLowerCase().match(this.search.replace(/[^\w\s]/gi, "").toLowerCase()) || item.gender.toLowerCase().match(this.search.replace(/[^\w\s]/gi, "").toLowerCase())
            })
            else
