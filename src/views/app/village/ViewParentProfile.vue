<template>
    <section class="section-main" v-if="is_village || is_super || is_sublevel_parents">
        ...
        <div class="flx gap-50 row-column">
            ...
            <chat-interface v-if="device != 'mobile'" :chats="chats" :to="parent.id" :name="parent.name" :image="parent.image" :message_id="message_id" @add-to-chat="addToChat" @set-message-id="setMessageId"/>
            <router-link v-else :to="{ name: 'MessageDetail', params: { to: this.$route.params.to, name: this.$route.params.parent }, replace: true}" class="bg-white gap-8 a-btn a-button ft-primary h-55 mb-32">
  ...
  components: { KidsList, ProfileBodyContent, ChatInterface, AccessDenied },
    ...
    methods: {
        async fetchThisParent() {
            try {
                const res = await postApi(this.hostname+'/api/fetch-this-parent?token='+ this.token, { parent: this.$route.params.to })
