    ...
    <teleport to="body">
        ...
        <transition name="drop-fade" mode="out-in">
            <div class="dropdown" v-if="toggleMenu" :style="{ left: dynamicFloatingDiv.left-80+'px', top: dynamicFloatingDiv.top+45 + 'px'}">
                <ul>
                    <sort-list v-for="list in sortList" :key="list.id" :list="list" @sort-item="doSort" />
                </ul>
            </div>
        </transition>
    </teleport>
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            sortList: (state) => state.data.sortList
        })
    },
    methods: {
        doSort(payload) {
            this.$store.commit('parentSort', payload)
            this.$emit('sortItem', payload)
