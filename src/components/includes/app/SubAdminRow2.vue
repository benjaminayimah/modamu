<template>
    <router-link :to="{ name: 'ViewParentProfile', params: { parent_id: this.user.id, parent: this.user.name } }" class="table-row grid-item row-hover grid-col-subadmins">
        ...
        <div class="table-cell"><span class="wrap-text wrap-line-1">{{ user.email }}</span></div>
        <div class="table-cell">{{ user.access_level }} <button @click.prevent="$store.commit('openAllocateVillage', user)">allocate</button></div>
        <div class="table-cell jc-sb">
            <div class="flx ai-c row-actions gap-8">
                <span>Edit</span>
                <span class="wrap-text wrap-line-1">Reset password</span>
