<template>
    <router-link :to="{ name: 'Attendee', params: { id: attendee.id,  name: attendee.kid_name } }" class="grid-item table-row">
        ...
        <div class="flx jc-c">
            <button v-if="attendee.status != '3'" @click.prevent="doChecking" class="button-primary gap-8" :class="{ 'check-out' : attendee.status == '2'}">
                <svg v-if="attendee.status == '0'" xmlns="http://www.w3.org/2000/svg" height="19" viewBox="0 0 33 33">
