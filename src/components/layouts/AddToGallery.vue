  ...
  <teleport to="#modal_content">
        <div>
            <div class="modal-title flx column ai-c gap-8 mb-24">
                <h3 id="modal_title">Add more pictures</h3>
            </div>
...
export default {
  components: { Avatar, Spinner },
    ...
    computed: {
        ...mapGetters(['getHostname', 'getToken', 'getUser']),
        ...mapState({
    ...
    methods: {
        doSubmit() {
            if(this.checkExist()) {
                this.creating = true
                axios.post(this.getHostname + '/api/add-to-gallery/'+this.forms.id +'?token='+this.getToken, this.form)
                .then(() => {
                    location.reload()
                }).catch((e) => {
                    this.creating = false
